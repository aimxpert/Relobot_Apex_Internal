...
namespace SDK
{
	namespace utils
	{
		namespace mono
		{
			...
			bool get_camera_and_local1(void* const gom, BaseCamera*& base_camera_ret, BasePlayer*& local_player_ret)
			{


				const auto last_object = *reinterpret_cast<unk1**>(gom);
				if (!last_object)
					return 0;


				const auto first_object = *reinterpret_cast<unk1**>(std::uintptr_t(gom) + 0x8);
				if (!first_object)
					return 0;
				for (auto object = first_object; object != last_object; object = object->next)
				{
					if (!object)
						return 0;
					auto base_object = *reinterpret_cast<uintptr_t**>(object + 0x10);
					if (!base_object)
						return 0;

					if (!first_object)
						return 0;
					auto mono_object = *reinterpret_cast<MonoObject**>(base_object + 0x30);
					if (!mono_object)
						return 0;
				...
				//	std::printf("tag %i  \n", tag);
					...
					//	const auto game_object = mono_object->object;
					//	std::printf(" 7 \n");
					...
					//	base_camera_ret = reinterpret_cast<BaseCamera*>(game_object->unk);
					...
					//	const auto base_player_ent =
					//		static_cast<BasePlayer*>(player_reference->entity);
					//	std::printf(" 20 \n");
					...
					//		base_player_ent->get_player_model_derivate();
					//	std::printf(" 02 \n");

					//	if (!player_model)
					//		continue;

					//	if (player_model->is_local_player())
					//		std::printf(" 000000000002 \n");
					//	{
					//		local_player_ret = base_player_ent;
					//		return true;
					//	}
					//}
			...
			bool get_camera_and_local(void* const gom, BasePlayer*& local_player_ret)
			{
				const auto last_object = *reinterpret_cast<unk1**>(gom);
				const auto first_object = *reinterpret_cast<unk1**>(std::uintptr_t(gom) + 0x8);
