...
namespace SDK
{
	namespace rendering
	{
		...
		public:
			...
			void DrawNameTag(float Size ,std::string name, DirectX::XMFLOAT2 screen_position, DirectX::XMFLOAT4 tag_color, float distance, bool healthbar = false, float health = 0.f);
			void DrawInventoryList(std::vector<std::string> text, std::string name, DirectX::XMFLOAT2 screen_resolution, DirectX::XMFLOAT4 tag_color);
			...
			void DrawBox2(const ImVec2& position, const ImVec2& headPosition, uint32_t color);
			void DrawLine2(const ImVec2& from, const ImVec2& to, uint32_t color, float thickness = 1.0f);
